...
**Keys are generated by DKG, with no dealer** ([ADR-0023](./docs/adr/0023-frost-dkg.md)).
The coordinator runs a 3-round Pedersen/Feldman ceremony across the five
participants via `POST /v1/frost/dkg/init`. Each participant samples its own
polynomial. Round-2 shares go through the coordinator sealed peer-to-peer
(X25519 + HKDF + ChaCha20-Poly1305, with peer keys pinned in `MPC_DKG_PEERS`).
Every share is checked against its sender's commitments, and a share is
installed only after all five agree on the group. No process ever holds a
user's or the house's private key, and the API receives only the address and
the verification shares. Keys created before this change were dealer-made;
see the ADR.
